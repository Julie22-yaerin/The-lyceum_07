...
- **Chat filters**: a segmented Primary / Unread / Read control on the chat inbox. Unread state seeds from `mockInitiallyUnread` and is cleared per-thread via `markThreadRead()` when a thread is actually opened (`app/(app)/chat/[friendId]/page.tsx`), persisted so the inbox reflects it after navigating back. Verified the full loop with Playwright: Unread showed Minh+Lan, opening Minh's thread and returning left only Lan.
...
- Achievements/filters/actions all verified with Playwright interaction tests (not just screenshots): like toggle changes count and fill, comment mention renders correctly, chat unread count updates after visiting a thread.
