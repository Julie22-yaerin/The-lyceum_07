...
## Firebase: auth wall + Firestore as the real database

The user supplied a real Firebase project (`pclick-9f190`) and asked for an email/Google login wall and Firestore to replace `localStorage`. Firebase API keys are not secrets by Firebase's own design (unlike the OpenRouter/Iframely keys — real access control lives in Firestore Security Rules and Firebase Auth, not in hiding this key), so they're committed to `.env`/`.env.example` the same as any other client config.

- `lib/firebase/client.ts`: initializes the Firebase app once (guarded against Next.js hot-reload re-init) and exports `auth`, `db` (Firestore), `googleProvider`, and a guarded `getFirebaseAnalytics()` (analytics needs a real browser and isn't supported in every context, so it's resolved lazily via `isSupported()` and is `null` on the server or in unsupported webviews).
- `components/AuthProvider.tsx`: wraps `onAuthStateChanged`, exposes `{ user, loading }` via context, mounted once in `app/layout.tsx` so both `/login` and the guarded app shell can read auth state.
- `components/AuthGuard.tsx`: client-side route guard (no Next.js middleware/session cookies — simpler given the time constraints of shipping this to a first user). Renders a blank `bg-bg` div while `loading`, redirects to `/login?next=<path>` once resolved with no user, otherwise renders children. Wraps `app/(app)/layout.tsx` (Reels/Chat/Profile) and `app/onboarding/page.tsx` directly (onboarding isn't under the `(app)` route group, so it needed its own guard).
- `app/login/page.tsx`: single page, toggles between sign-in/sign-up. Email+password via `createUserWithEmailAndPassword`/`signInWithEmailAndPassword`, Google via `signInWithPopup(auth, googleProvider)`. New accounts (checked via `getAdditionalUserInfo(result)?.isNewUser` for Google, and simply "just called `createUserWithEmailAndPassword`" for email) redirect to `/onboarding?next=<original target>`; existing users go straight to their target. A `handledRedirectRef` skips the page's generic "already logged in → redirect" effect during these explicit post-auth redirects, avoiding a race where that effect would bounce a brand-new signup straight to `/feed` before `/onboarding` had a chance to navigate.
- Landing page and invite-page CTAs (`app/page.tsx`, `app/invite/[code]/page.tsx`) now point at `/login` instead of `/onboarding`/`/feed` directly, since both of those are now behind the wall.
- Profile gained a **Sign out** row (`signOut(auth)` then redirect to `/login`).

**`lib/storage.ts` fully rewritten from `localStorage` to Firestore** — every function is now `async`, keyed off `auth.currentUser.uid`:
- `users/{uid}` — profile doc (`getStoredProfile`/`saveStoredProfile`, `setDoc(..., { merge: true })`).
- `users/{uid}/readThreads/{friendId}` and `users/{uid}/likedVideos/{videoId}` — existence-based per-user state (doc exists = read/liked; `deleteDoc` to unlike).
- `users/{uid}/messageThreads/{friendId}` — `{ items: ChatMessage[] }` with `arrayUnion` for shared-video messages sent from the Reels action rail.
- `comments/{commentId}` — top-level collection, **not** per-user like the rest: comments are meant to be visible to everyone (like real video comments), so they're written once with a client-generated ID and fetched as a flat collection scan, grouped by `videoId` client-side. This is a v1-scale simplification (no pagination, no per-video query) that will need revisiting if comment volume grows.
- All 5 call sites (`app/onboarding/page.tsx`, `app/(app)/profile/page.tsx`, `components/feed/VideoActions.tsx`, `app/(app)/chat/page.tsx`, `app/(app)/chat/[friendId]/page.tsx`) updated from sync reads to `.then()`/`await`. Optimistic UI updates (like-toggle, new comment, new chat message) still update local state immediately and fire the Firestore write in the background (`void`).

`firestore.rules` (+ minimal `firebase.json`/`.firebaserc` pointing at `pclick-9f190`) written for the schema above: every `users/{uid}` doc and subcollection is locked to `request.auth.uid == uid`; `comments` are readable by any signed-in user, `create`-only (never `update`/`delete`) with field-shape validation, matching the app's real behavior (no edit/delete UI exists for comments). **This session has no Firebase CLI credentials for `pclick-9f190`, so the rules are written but not deployed** — run `firebase deploy --only firestore:rules` from a machine logged into that project (or paste `firestore.rules` into the Firebase Console's Rules editor) before shipping to a real user, otherwise Firestore's default rules (test-mode open, or locked-closed depending on how the project was created) apply instead of these.

**Live-verified**: `curl` (through this sandbox's proxy, `--cacert /root/.ccr/ca-bundle.crt`) called the real `identitytoolkit.googleapis.com` signup endpoint with the project's actual API key and got back a genuine `idToken`/`refreshToken`/`localId` — confirms the Firebase project, API key, and Auth product are all correctly wired end-to-end. **Note**: this created one real throwaway `test@example.com` account in `pclick-9f190`'s Auth users list as a side effect of that verification — harmless, but worth deleting from the Firebase Console before shipping if a clean user list matters.

**Not fully verified end-to-end through the UI**: a full click-through Playwright test (sign up → onboarding → feed → profile → sign out → sign back in) was attempted but blocked by a sandbox-only limitation — Chromium refuses to complete *any* HTTPS request through this session's proxy (`ERR_CONNECTION_RESET`, reproduced even on `https://example.com`, unrelated to Firebase specifically), while `curl` through the identical proxy works fine. This is very likely the same class of "some clients don't route through HTTPS_PROXY correctly" issue noted in `/root/.ccr/README.md` for Node's fetch (which needed `NODE_USE_ENV_PROXY=1`), just with no equivalent flag documented for Chromium in this environment — not chased further given time constraints and that it's clearly sandbox-specific, not an app bug. What *was* verified with a real Playwright browser (this part needs no external HTTPS, only the local Next.js server): every protected route (`/feed`, `/chat`, `/onboarding`) correctly redirects an unauthenticated visitor to `/login?next=<original path>`; public routes (`/`, `/login`) render correctly with working Google/email UI. The Google sign-in popup flow specifically should get a real click-through test once this is deployed somewhere with a stable origin, since `signInWithPopup` requires the deployed domain to be in the Firebase project's **Authorized domains** list (Console → Authentication → Settings) — `localhost` is allowlisted by default but the Railway production domain will need to be added manually.

## AI-picked YouTube Short fallback for TikTok/Instagram

Replaces the previous last-resort tier (the platform's own oEmbed `<blockquote>`+`<script>` widget from bug-fix #43 above) with a native YouTube embed of a same-topic Short, per the user's explicit request — the oEmbed widget is fragile/embed-restricted in a lot of real-world contexts, whereas a YouTube iframe embed is the one player type already proven fully reliable in this app.

- `lib/media/youtube-fallback.ts`: `resolveTopicYouTubeShort(topic)` — OpenRouter turns a topic id (e.g. `"igcse-calculus"`) into a short natural-language search phrase (same JSON-mode pattern as the meme subreddit picker; falls back to the raw topic id on any AI failure), then scrapes the first `videoId` out of a `youtube.com/results?...` search page (no official Data API key configured for this project, so this is a best-effort HTML scrape — regex-extracted from the page's embedded JSON, not a stable API contract). Returns a ready-to-use `youtube.com/embed/...` URL, or `null` on any failure (never throws).
- `lib/types.ts`: `ResolveRequestBody` gained an optional `topic` field; `components/feed/VideoSlide.tsx` now sends `video.topic_id` on every resolve call.
- `app/api/v1/media/resolve/route.ts` reordered: real playable results (Iframely `direct_mp4`/`iframe`, then Microlink `direct_mp4`) still win first exactly as before; only when *neither* provider produced a real playable video does a `tiktok`/`instagram` request with a `topic` try the YouTube Short fallback; the oEmbed-widget/`iframe_fallback` tier is now the true last resort, used only if the YouTube fallback itself fails (no AI, no search match) or no `topic` was sent.
- **Live-verified end-to-end** (network open in this session): a deliberately broken/nonexistent TikTok URL correctly fell through to a real YouTube Short (`_KY-Up86CVk`) for topic `igcse-calculus`. More notably, the **already-real, previously-working TikTok URL** (`@scout2015/...`, the one whose oEmbed widget bug-fix #43 fixed) *also* now returns the YouTube Short instead of the widget — confirmed via YouTube's own oEmbed endpoint that the returned video is real and genuinely on-topic ("Math is EASY: Basic Calculus"). This is the intended behavior per the request (swap the fallback tier, not just the broken-link case) — worth flagging in case a future session expected the real TikTok oEmbed widget to still show for that URL. Also re-verified native YouTube links (`platform: "youtube"`) are completely unaffected — they still resolve directly via `extractYouTubeVideoId`, never touching this fallback path at all.
